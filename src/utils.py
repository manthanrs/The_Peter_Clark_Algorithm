...
def audit_graph_differences(truth, est, node_names=None):
    ...
    if node_names is None:
        # Changed {i} to {i+1} to shift from 0-based to 1-based indexing
        node_names = [f"Node {i+1}" for i in range(len(truth))]
...
def plot_cpdag(adj_matrix, labels, title="Recovered CPDAG", figsize=(10, 8)):
    ...
    # Define the consistent node size
    LARGE_NODE_SIZE = 3000

    # Draw nodes
    nx.draw_networkx_nodes(G, pos, node_size=LARGE_NODE_SIZE, node_color='skyblue')

    # Draw labels
    nx.draw_networkx_labels(G, pos, font_size=22, font_weight='bold')

    # Draw undirected edges (blue, no arrows)
    # Passed LARGE_NODE_SIZE so edges align correctly with the node boundaries
    if undirected_edges:
        nx.draw_networkx_edges(G, pos, edgelist=undirected_edges, edge_color='blue', 
                               arrows=False, style='solid', width=3.0, 
                               node_size=LARGE_NODE_SIZE)

    # Draw directed edges (green, with arrows)
    # Passed LARGE_NODE_SIZE so arrows are pushed outside the node boundaries
    if directed_edges:
        nx.draw_networkx_edges(G, pos, edgelist=directed_edges, edge_color='green', 
                               arrows=True, arrowsize=25, width=3.0, 
                               node_size=LARGE_NODE_SIZE)

...
def nonlinear_permutation_test(df, X, Y, condition_set=None, n_permutations=999, random_seed=42):
    ...
    # --- Case 2: Conditional dependence via nonlinear residualization
        ...
        rf_x = RandomForestRegressor(
            n_estimators=1000, min_samples_leaf=10,min_samples_split=20,max_features=current_max_features,bootstrap=True,max_samples=0.75,max_depth=None, random_state=random_seed, ccp_alpha=0,oob_score=True, n_jobs=-1
        )
        ...
        rf_x.fit(Z, A.ravel())
        rf_y.fit(Z, C.ravel())      
        res_X = A.ravel() - rf_x.oob_prediction_  # The oob predictions are already stored. So do no predict
        res_Y = C.ravel() - rf_y.oob_prediction_
        """plt.figure()
        plt.plot(res_X)
        plt.plot(res_Y)

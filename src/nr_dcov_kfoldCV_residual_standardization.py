...
def nonlinear_permutation_test(df, X, Y, condition_set=None, n_permutations=999, random_seed=42):
    ...
    # --- Case 2: Conditional dependence via nonlinear residualization
        ...
        if conditioning_set_size <= 3:
           current_max_features = 1.0  # Use all available conditioned variables
        else:
           current_max_features = 0.5  # Force tree decorrelation
                   
        rf_x = RandomForestRegressor(
              n_estimators=1000, min_samples_leaf=10, min_samples_split=20,max_features=current_max_features,bootstrap=True,max_samples=0.75,random_state=random_seed, ccp_alpha=0,n_jobs=-1
        )
        ...
        pred_sum_x = np.zeros(len(A1d),dtype=float)
        pred_count = np.zeros(len(A1d),dtype=float)
        pred_sum_y = np.zeros(len(C1d),dtype=float)
       
        #Custom loop to evaluate combinations
        for split_id, (train_idx,test_idx) in enumerate(rkf.split(Z)):
            clone_rf_x = clone(rf_x)  #make copies of the RF models
            clone_rf_y = clone(rf_y)
            
            #Provide each cloned model a different seed
            clone_rf_x.set_params(random_state=42 + split_id)
            clone_rf_y.set_params(random_state=42 + split_id)
            
            clone_rf_x.fit(Z[train_idx], A1d[train_idx]) #using the A1d and C1d arrays (instead of A and C which are numpy arrays)
            clone_rf_y.fit(Z[train_idx], C1d[train_idx])
            
            pred_sum_x[test_idx] += clone_rf_x.predict(Z[test_idx])  #prediction on the test data set
            pred_sum_y[test_idx] += clone_rf_y.predict(Z[test_idx])
            pred_count[test_idx] += 1
            
        Xhat = pred_sum_x/pred_count
        Yhat = pred_sum_y/pred_count
        raw_res_X = A.ravel() - Xhat
        raw_res_Y = C.ravel() - Yhat
        
        #3. FIX THE VOLATILITY (Heteroscedasticity adjustement)
        # Fit a secondary RF model to the squared residuals to model how thhe variance scales with Z
        # First we take our raw residuals and square them (raw_res_X2). Why? Because and error of 
        #-5 cm and +5 cm representa the exact same amount of volatility. Squaring them makes everying
        #positive so we are only looking at the pure magnitude of the noise
        rf_var_x = RandomForestRegressor(
              n_estimators=1000, min_samples_leaf=10, min_samples_split=20,max_features=current_max_features,bootstrap=True,max_samples=0.75,random_state=random_seed, ccp_alpha=0,n_jobs=-1
        )
        
        rf_var_y = RandomForestRegressor(
              n_estimators=1000, min_samples_leaf=10, min_samples_split=20,max_features=current_max_features,bootstrap=True,max_samples=0.75,random_state=random_seed, ccp_alpha=0,n_jobs=-1
        )
        rkf = RepeatedKFold(
             n_splits=5,n_repeats=5,random_state=42
        ...
        pred_sum_var_x = np.zeros(len(raw_res_X),dtype=float)
        pred_count = np.zeros(len(raw_res_X),dtype=float)
        pred_sum_var_y = np.zeros(len(raw_res_Y),dtype=float)
       
        #Custom loop to evaluate combinations
        for split_id, (train_idx,test_idx) in enumerate(rkf.split(Z)):
            clone_rf_var_x = clone(rf_var_x)  #make copies of the RF models
            clone_rf_var_y = clone(rf_var_y)
            
            #Provide each cloned model a different seed
            clone_rf_var_x.set_params(random_state=42 + split_id)
            clone_rf_var_y.set_params(random_state=42 + split_id)
            
            clone_rf_var_x.fit(Z[train_idx], raw_res_X[train_idx]**2) #using the A1d and C1d arrays (instead of A and C which are numpy arrays)
            clone_rf_var_y.fit(Z[train_idx], raw_res_Y[train_idx]**2)
            
            pred_sum_var_x[test_idx] += clone_rf_var_x.predict(Z[test_idx])  #prediction on the test data set
            pred_sum_var_y[test_idx] += clone_rf_var_y.predict(Z[test_idx])
            pred_count[test_idx] += 1
            
        #Predict variance of res_X and res_Y
        pred_var_rf_Xhat = pred_sum_var_x/pred_count
        pred_var_rf_Yhat = pred_sum_var_y/pred_count
        
        # Standardize the residuals by their predicted standard deviation
        res_X = raw_res_X / np.sqrt(pred_var_rf_Xhat)
        res_Y = raw_res_Y / np.sqrt(pred_var_rf_Yhat)
        ...
        r_permuted = np.zeros(n_permutations)
        for i in range(n_permutations):
            Y_perm = np.random.permutation(res_Y)
            r_permuted[i] = dcor.distance_covariance(res_X, Y_perm)

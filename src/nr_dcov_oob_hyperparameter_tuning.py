...
def nonlinear_permutation_test(df, X, Y, condition_set=None, n_permutations=500, random_seed=42):
    ...
    # --- Case 2: Conditional dependence via nonlinear residualization
        ...
        best_score_rf_x = -np.inf
        best_params_rf_x = {}
        best_rf_x = None  # Visual anchor: This will hold your final usable model

        best_score_rf_y = -np.inf
        best_params_rf_y = {}
        best_rf_y = None  # Visual anchor: This will hold your final usable model

        #Custom loop to evaluate combinations
        for max_depth in grid_search_params['max_depth']:
            for ccp_alpha in grid_search_params['ccp_alpha']:
                #print('max_depth = ',max_depth, '; ccp_alpha = ',ccp_alpha)
            	# Nonlinear regression using Random Forests
                rf_x = RandomForestRegressor(
                      n_estimators=200, max_depth=max_depth, random_state=random_seed, ccp_alpha=ccp_alpha,oob_score=True, n_jobs=-1
                )
                ...
                current_oob_score_rf_x = rf_x.oob_score_  
                current_oob_score_rf_y = rf_y.oob_score_  
        
                # Track the best configuration and save the model object
                if current_oob_score_rf_x > best_score_rf_x:
                    best_score_rf_x = current_oob_score_rf_x;
                    best_params_rf_x = {'max_depth': max_depth, 'ccp_alpha': ccp_alpha}
                    # Deep copy ensures we save a standalone snapshot of this specific model
                    best_rf_x = copy.deepcopy(rf_x)
            
                if current_oob_score_rf_y > best_score_rf_y:
                    best_score_rf_y = current_oob_score_rf_y;
                    best_params_rf_y = {'max_depth': max_depth, 'ccp_alpha': ccp_alpha}
                    # Deep copy ensures we save a standalone snapshot of this specific model
                    best_rf_y = copy.deepcopy(rf_y)    
        print('RF: X: best_oob_score = ',best_score_rf_x,' best_params', best_params_rf_x)
        print('RF: Y: best_oob_score = ',best_score_rf_y,' best_params', best_params_rf_y)
        rf_x = best_rf_x
        rf_y = best_rf_y
